...
import com.regy.quantalink.flink.core.streaming.operator.process.JoinProcess;
...
public class IntervalJoinByUnionKey<K, T1 extends UserDefinedTimeKey<T1>, T2 extends UserDefinedTimeKey<T2>, OUT>
    ...
    public IntervalJoinByUnionKey(JoinProcess<K, T1, T2, OUT> joinFunction, Long rightTtl, OutputTag<Tuple2<T1, T2>> outputTag, Class<T1> leftClass, Class<T2> rightClass) {
    ...
    public void open(Configuration parameters) {
        leftBuffer = this.getRuntimeContext().getMapState(new MapStateDescriptor<>(LEFT_BUFFER, Types.LONG, Types.MAP(Types.STRING, Types.LIST(leftTypeInformation))));
        rightBuffer = this.getRuntimeContext().getMapState(new MapStateDescriptor<>(RIGHT_BUFFER, Types.LONG, Types.MAP(Types.STRING, Types.LIST(rightTypeInformation))));
    }

    @Override
    public void onTimer(long timestamp, KeyedCoProcessFunction<K, T1, T2, OUT>.OnTimerContext ctx, Collector<OUT> collector) throws Exception {
    ...
    @Override
    public void processElement1(T1 record, KeyedCoProcessFunction<K, T1, T2, OUT>.Context ctx, Collector<OUT> collector) throws Exception {
        this.processElement(record, leftBuffer, rightBuffer, true, ctx, collector);
    }

    @Override
    public void processElement2(T2 record, KeyedCoProcessFunction<K, T1, T2, OUT>.Context ctx, Collector<OUT> collector) throws Exception {
        this.processElement(record, rightBuffer, leftBuffer, false, ctx, collector);
    }

    private <THIS, OTHER extends UserDefinedTimeKey<OTHER>> void processElement(UserDefinedTimeKey<THIS> record, MapState<Long, Map<String, List<THIS>>> ourBuffer, MapState<Long, Map<String, List<OTHER>>> otherBuffer, boolean isLeft, KeyedCoProcessFunction<K, T1, T2, OUT>.Context ctx, Collector<OUT> collector) throws Exception {
        ...
        if (currentWatermark < cleanupTimestamp) {
            ...
            while (true) {
                ...
                if (otherList != null) {
                    for (OTHER otherValue : otherList) {
                        leftLowerBound = isLeft ? record.getLowerBound() : otherValue.getLowerBound();
                        if (leftLowerBound < rightTimestamp) {
                            if (isLeft) {
                                collector.collect(joinFunction.join(leftClass.cast(value), rightClass.cast(otherValue)));
                            } else {
                                collector.collect(joinFunction.join(leftClass.cast(otherValue), rightClass.cast(value)));
                            }
                        }
                    }
                }
            ...
            ctx.output(outputTag, isLeft ? new Tuple2<>(leftClass.cast(value), null) : new Tuple2<>(null, rightClass.cast(value)));
        }
    }

    private static <T> void addToBuffer(MapState<Long, Map<String, List<T>>> buffer, T value, long timestamp, String key) throws Exception {
