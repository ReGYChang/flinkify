...
public class FlinkStreamingContext {
    ...
    public <T> SourceConnector<T> getSourceConnector(String connectorId, TypeInformation<T> typeInformation) {
        return Optional.ofNullable(
                        (SourceConnector<T>) sourceConnectors.get(
                                new ConnectorKey<>(connectorId, typeInformation)))
                .orElseThrow(() ->
                        new FlinkException(
    ...
    public <I, O> SinkConnector<I, O> getSinkConnector(
    ...
    @SuppressWarnings("unchecked")
    public <I, O> SinkConnector<I, O> getSinkConnector(
            String connectorId, TypeInformation<I> inputTypeInfo, TypeInformation<O> outputTypeInfo) {
        return Optional.ofNullable(
                        (SinkConnector<I, O>) sinkConnectors.get(
                                new ConnectorKey<>(connectorId, inputTypeInfo)))
                .orElseThrow(
                        () ->
                                new FlinkException(
                                        ErrCode.STREAMING_CONNECTOR_FAILED,
                                        String.format(
                                                "Sink connector could not be found with input type '%s' & output type '%s'",
                                                inputTypeInfo,
                                                outputTypeInfo)));
    }

    public <T> DataStreamSource<T> getSourceDataStream(TypeInformation<T> typeInformation) throws FlinkException {
        return Optional.ofNullable(getSourceConnector(typeInformation).getSourceDataStream())
                .orElseThrow(
                        () ->
                                new FlinkException(
                                        ErrCode.STREAMING_CONNECTOR_FAILED,
                                        String.format(
                                                "Source data stream could not be found with type: [%s]",
                                                typeInformation)));
    }

    public <T> DataStreamSource<T> getSourceDataStream(
            String connectorId, TypeInformation<T> typeInformation) throws FlinkException {
        return Optional.ofNullable(getSourceConnector(connectorId, typeInformation).getSourceDataStream())
                .orElseThrow(
                        () ->
                                new FlinkException(
                                        ErrCode.STREAMING_CONNECTOR_FAILED,
    ...
    public <I, O> DataStreamSink<O> getSinkDataStream(
            String connectorId, TypeInformation<I> inputTypeInfo, TypeInformation<O> outputTypeInfo, DataStream<I> stream) {
    ...
    @SuppressWarnings("UnusedReturnValue")
    public <I, O> DataStreamSink<O> getSinkDataStream(
            TypeInformation<I> inputTypeInfo, TypeInformation<O> outputTypeInfo, DataStream<I> stream) {
