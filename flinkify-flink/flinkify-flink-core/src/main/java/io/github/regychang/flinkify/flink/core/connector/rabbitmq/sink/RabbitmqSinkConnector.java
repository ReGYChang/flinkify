...
import io.github.regychang.flinkify.flink.core.connector.rabbitmq.serialization.RabbitmqSerializationAdapter;
    ...
    public DataStreamSink<T> createSinkDataStream(DataStream<T> stream) {
        try {
            ...
            throw new FlinkException(
                    ...
                    String.format(
                            "Rabbitmq sink connector '%s' serialization adapter must be '%s', " +
                                    "could not assign other serialization adapter",
                            getName(), RabbitmqSerializationAdapter.class), e);
        } catch (Exception e) {
            throw new FlinkException(
                    ErrCode.STREAMING_CONNECTOR_FAILED,
                    String.format("Failed to initialize RabbitMQ sink connector '%s'", getName()), e);
        }
    }
}
