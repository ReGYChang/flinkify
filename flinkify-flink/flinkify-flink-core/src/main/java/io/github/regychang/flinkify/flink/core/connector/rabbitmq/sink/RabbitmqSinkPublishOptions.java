package io.github.regychang.flinkify.flink.core.connector.rabbitmq.sink;
...
public class RabbitmqSinkPublishOptions<T> implements RMQSinkPublishOptions<T> {
    ...
    public RabbitmqSinkPublishOptions(Configuration config, ComputePropertiesFunc<T> computePropertiesFunc) {
        ...
        this.routingKey =
                config.getNotNull(
                        RabbitmqOptions.ROUTING_KEY,
                        String.format("Rabbitmq sink connector '%s' routing-key must not be null", sinkName));
        this.exchange =
                config.getNotNull(
                        RabbitmqOptions.EXCHANGE,
                        String.format("Rabbitmq sink connector '%s' exchange must not be null", sinkName));
        this.mandatory =
                config.getNotNull(
                        RabbitmqOptions.MANDATORY,
                        String.format("Rabbitmq sink connector '%s' mandatory must not be null", sinkName));
        this.immediate =
                config.getNotNull(
                        RabbitmqOptions.IMMEDIATE,
                        String.format("Rabbitmq sink connector '%s' immediate must not be null", sinkName));
