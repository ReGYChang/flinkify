...
import io.github.regychang.flinkify.flink.core.connector.nebula.operator.GraphProcessFunc;
...
public class GenerateSensorMapGraph extends GraphProcessFunc<SensorMap> {
    ...
    private void extractProductionLineInfo(
            SensorMap.ProductionLine line,
            ProcessFunction<SensorMap, Void>.Context ctx,
    ...
    private Map<String, Tuple2<Integer, SensorMap.SequenceInfo>> extractSequenceInfo(
            List<SensorMap.SequenceInfo> sequence) {
    ...
    private Row createStationRow(
            SensorMap.Workstation station,
            Integer rowArity,
            Tuple2<Integer, SensorMap.SequenceInfo> sequenceTuple) throws NoSuchAlgorithmException {
        Row row = new Row(rowArity);
